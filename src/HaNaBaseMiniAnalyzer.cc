  ...
  SampleName( iConfig.getParameter< string >("sample") )
{
  ...
  if (iConfig.exists( "Tracks")){
    packedReader = new PackedCandidateReader( iConfig.getParameter< edm::ParameterSet >("Tracks") , consumesCollector() , vertexReader );
    lostReader = new PackedCandidateReader( iConfig.getParameter< edm::ParameterSet >("LostTracks") , consumesCollector() , vertexReader);
  }
  ...
  std::cout << "HaNaBaseMiniAnalyzer" << std::endl;
